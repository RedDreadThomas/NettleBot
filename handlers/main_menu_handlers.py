...
@dp.callback_query_handler(lambda c: c.data == 'main_menu1' or c.data == "end_words_game2"
...
async def choose_mod(call: types.CallbackQuery):
    ...
    :return:
    """
    keyboard = logic.Markup().pull([emojize(':brain: Развить память'), emojize(':no_entry: Запомнить новое'),
                                    emojize(':no_entry: Повторить старое'), emojize(':scroll: Вернуться в меню')],
                                   'menu')
    await call.message.answer('Выберите один из режимов бота, краткое описание каждого из нх вы можете найти в'
...
async def information(call: types.CallbackQuery):
    ...
    :return:
    """
    keyboard = logic.Markup().pull([emojize(':scroll: В меню')], 'exit_from_info')
    await call.message.answer("Крапивка будет способна работать в 3 режимах:\n"
                              "1)Развить память.\n"
...
async def callback(call: types.CallbackQuery):
    ...
    :return:
    """
    keyboard = logic.Markup().pull([emojize(':scroll: В меню')], 'exit_from_communication')
    await call.message.answer("Если у вас есть предложения или жалобы, можете смело писать нам:\n"
                              "На почту: скоро\n"
                              "В группу в Вк: скоро\n"
